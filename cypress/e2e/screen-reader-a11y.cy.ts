...
describe('Screen Reader Accessibility Tests', () => {
  ...
  it('Should pass screen reader specific accessibility rules', () => {
    ...
    }, undefined, true); // Skip failures as we're just checking
  });
  ...
  it('Form elements should have associated labels', () => {
    ...
    cy.get('.profile-modal input, .profile-modal select, .profile-modal textarea').each($field => {
  ...
  it.skip('ARIA attributes should be used correctly', () => {
    ...
    cy.get('[role="button"]').each($el => {
      expect($el.prop('tagName') === 'BUTTON' || 
             $el.is('[tabindex]') || 
             $el.css('cursor') === 'pointer' ||
             $el.is('a[href]')).to.be.true; 
    });
    
    cy.get('#your-snake-info .editable-profile-item').first().click();
    cy.get('div.ReactModal__Content[role="dialog"]').should('exist').and('have.attr', 'aria-labelledby', 'profile-modal-title');
