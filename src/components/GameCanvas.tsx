...
import { GameState, Snake, Food, PowerUp, Point } from '../game/state/types';
...
const GameCanvas: React.FC<GameCanvasProps> = ({ gameState, localPlayerId }) => {
  ...
  // Draw snake eyes (memoized)
  const drawSnakeEyes = useCallback(
    (ctx: CanvasRenderingContext2D, headPosition: Point, direction: string) => {
  ...
  // Draw a snake on the canvas (memoized)
  const drawSnake = useCallback(
    (ctx: CanvasRenderingContext2D, snake: Snake, isLocalSnake: boolean) => {
  ...
  }, [gameState, localPlayerId, drawSnake, drawFood, drawPowerUp]);
